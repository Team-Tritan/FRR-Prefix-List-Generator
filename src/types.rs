...
impl Asn {
    ...
    pub const fn is_valid_public(&self) -> bool {
        let is_private_16bit = self.0 >= Self::PRIVATE_16BIT_START 
            && self.0 <= Self::PRIVATE_16BIT_END;
        let is_private_32bit = self.0 >= Self::PRIVATE_32BIT_START 
            && self.0 <= Self::PRIVATE_32BIT_END;
